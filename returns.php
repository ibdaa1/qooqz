...
try {
    ...
    switch ($method) {
        ...
        case 'POST':
            ...
            ResponseFormatter::success(['id' => $newId], 'Return request created successfully', 201);
            break;

        case 'PUT':
            $data      = json_decode(file_get_contents('php://input'), true) ?: [];
            $updatedId = $controller->update($tenantId, $data);
            ResponseFormatter::success(['id' => $updatedId], 'Return request updated successfully');
            break;
    ...
    safe_log('warning', 'returns.validation', ['error' => $e->getMessage()]);
    ResponseFormatter::error($e->getMessage(), 422);
} catch (\RuntimeException $e) {
    safe_log('error', 'returns.runtime', ['error' => $e->getMessage()]);
    ResponseFormatter::error($e->getMessage(), 400);
} catch (\Throwable $e) {
    safe_log('critical', 'returns.fatal', ['error' => $e->getMessage(), 'trace' => $e->getTraceAsString()]);
