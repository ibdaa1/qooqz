<?php
/**
 * Template: Arabic Non-GCC Certificate  (ar_non_gcc)
...
$lang       = $lang ?? 'ar';
// In PDF mode: use Amiri (registered via registerFont API) + DejaVu Sans fallback.
// Exclude Arial (DB font_family) from PDF stack — it does not exist in dompdf.
...
<div class="page">
    ...
    <div class="content">
        <div class="cert-header">
            ...
            <h2><?= $lbl('non_gcc', 'خارج دول مجلس التعاون') ?></h2>
        </div>
        <div class="info-row"><span><?= $lbl('certificate_number', 'رقم الشهادة') ?>:</span> <?= $certNumber ?></div>
        <div class="info-row"><span><?= $lbl('issue_date', 'تاريخ الإصدار') ?>:</span> <?= $issuedAt ?></div>
        <div class="info-row"><span><?= $lbl('exporter', 'المصدّر') ?>:</span> <?= $exporterName ?></div>
        <div class="info-row"><span><?= $lbl('importer', 'المستورد') ?>:</span> <?= $importerName ?></div>
        <div class="info-row"><span><?= $lbl('destination_country', 'بلد الوجهة') ?>:</span> <?= $importerCountry ?></div>
        ...
        <div style="margin-top:3mm;font-size:8pt;color:#888;text-align:center;">
            <?= $lbl('verification_code', 'رمز التحقق') ?>: <?= htmlspecialchars($data['verification_code']) ?>
        </div>
        <?php endif; ?>
    </div>
</div>
