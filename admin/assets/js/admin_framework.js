 ...
 * 
 * ⚡ Fully dynamic – uses CSS classes only.
 * ⚡ No hardcoded colors / sizes – all theming comes from CSS variables.
 * ⚡ Compatible with database‑driven themes.
 */
(function() {
    'use strict';
    ...
    AF.$$ = function(selector, parent = document) { return parent.querySelectorAll(selector); };
    ...
    AF.notify = function(message, type = 'info') {
        ...
        setTimeout(() => notification.classList.add('af-notification-show'), 10);
        setTimeout(() => {
    ...
    AF.Table = {
        showLoading: function(config) {
            if (config.loading) config.loading.style.display = 'flex';
            if (config.container) config.container.style.display = 'none';
            if (config.empty) config.empty.style.display = 'none';
            if (config.error) config.error.style.display = 'none';
        },
        showTable: function(config) {
            if (config.loading) config.loading.style.display = 'none';
            if (config.container) config.container.style.display = 'block';
            if (config.empty) config.empty.style.display = 'none';
            if (config.error) config.error.style.display = 'none';
        },
        showEmpty: function(config) {
            if (config.loading) config.loading.style.display = 'none';
            if (config.container) config.container.style.display = 'none';
            if (config.empty) config.empty.style.display = 'flex';
            if (config.error) config.error.style.display = 'none';
        },
        showError: function(config, message = 'An error occurred') {
            ...
            if (config.container) config.container.style.display = 'none';
            if (config.empty) config.empty.style.display = 'none';
            if (config.error) config.error.style.display = 'flex';
            if (config.errorMessage) config.errorMessage.textContent = message;
        },
        render: function(tbody, rows) {
            if (!tbody) { console.error('[AF.Table.render] tbody element not found'); return; }
            let html = '';
            if (Array.isArray(rows)) html = rows.join('');
            else if (typeof rows === 'string') html = rows;
            else { console.error('[AF.Table.render] rows must be array or string'); return; }
        ...
        renderPagination: function(paginationEl, infoEl, meta) {
            ...
            let html = `<button class="pagination-btn" ${page === 1 ? 'disabled' : ''} data-page="${page - 1}"><i class="fas fa-chevron-left"></i> Previous</button>`;
            const maxVisible = 7;
            ...
            for (let i = startPage; i <= endPage; i++) {
                html += `<button class="pagination-btn ${i === page ? 'active' : ''}" data-page="${i}" ${i === page ? 'disabled' : ''}>${i}</button>`;
            }
            if (endPage < totalPages) {
                if (endPage < totalPages - 1) html += '<span class="pagination-dots">...</span>';
                html += `<button class="pagination-btn" data-page="${totalPages}">${totalPages}</button>`;
            }
            html += `<button class="pagination-btn" ${page === totalPages ? 'disabled' : ''} data-page="${page + 1}">Next <i class="fas fa-chevron-right"></i></button>`;
    ...
    AF.Modal = {
        ...
        alert: function(message, type = 'info') { AF.notify(message, type); }
    };
    ...
    // Badge helper – returns HTML with CSS class only
    AF.badge = function(text, type = 'info') {
        const typeMap = {
            success: 'badge-success', error: 'badge-danger', danger: 'badge-danger',
            warning: 'badge-warning', info: 'badge-info', primary: 'badge-primary', secondary: 'badge-secondary'
        };
        const className = typeMap[type] || 'badge-info';
        return `<span class="badge ${className}">${AF.escapeHtml(text)}</span>`;
    };

    // Action button helper – returns HTML with CSS classes
    AF.actionButton = function(icon, onclick, title = '', variant = 'primary') {
        const className = variant ? `btn btn-sm btn-${variant}` : 'btn btn-sm';
        return `<button onclick="${onclick}" class="${className}" title="${AF.escapeHtml(title)}"><i class="fas fa-${icon}"></i></button>`;
    };
    ...
    AF.Cache = {
        ...
        clear: function(key) { key ? this.data.delete(key) : this.data.clear(); }
    };
