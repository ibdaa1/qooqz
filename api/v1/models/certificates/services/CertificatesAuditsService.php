...
final class CertificatesAuditsService
{
    private PdoCertificatesAuditsRepository $repo;
    private CertificatesAuditsValidator $validator;

    public function __construct(
        PdoCertificatesAuditsRepository $repo,
        CertificatesAuditsValidator $validator
    ) {
        $this->repo = $repo;
    ...
    public function delete(int $tenantId, int $id): void
    {
        if (!$this->repo->delete($tenantId, $id)) {
            throw new RuntimeException('Failed to delete audit record.');
        }
    }
}